...
**Use `dense`.** `n_bud` is 200,000 and the binding rule requires every
cloud to exceed it (see `ablation_design.md` §5). Measured with `dense`:
299,368 / 334,931 / 356,674 / 471,531 points. `sparse` would give a
quarter of that, below the budget, which makes M2 inert under M1 and
collapses A4 onto A1 and A7 onto A5.
...
| `n_bud` | **200 000** | set before S1 from the binding rule, not fitted to A0 afterwards — it must lie below the dense cloud or M2 is inert under M1 |
