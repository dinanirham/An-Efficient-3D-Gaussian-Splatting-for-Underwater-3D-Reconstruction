...
| **A2** | **The scale discontinuity.** Pruning changes `Ẑ_min`/`Ẑ_max`, rescaling the medium model's only input mid-training → **D-2 arrives as an injected step change** | M-1/M-2/M-4 all defend D-2 against *optimization* exploiting it, none defends against an *externally injected* rescale | ❌ **no baseline mechanism covers this case** | **medium-only re-warm-up burst immediately after each simplification event** `[PI]`; log `Ẑ_min`, `Ẑ_max` and `β` across the event | ⚠️ **RISK REALISED, MITIGATION FAILED** `[measured n=12 + 12 control]`. All 12 A2 runs place their largest attenuation drop on a simplification boundary; 0 of 12 A0 runs lose a channel. The 200-step burst did **not** restore β in any of the six collapsed runs — the `rewarm_end` row *is* the post-burst state and β is already collapsed in it `[13-campaign-addendum §13.10, §13.13; 03-claim-validity-ledger E.2, E.6]` |
...
| **A4** | **Degenerate cell.** If M1's converged count already sits below M2's budget, IP2 is a no-op and **A4 ≡ A1** | n/a — a design failure, not a degeneracy | n/a | `n_bud` fixed **pre-campaign** below the smallest cloud, not from A0 — see the note below the table; preflight refuses any M1+M2 run where it would not bind, reading the count from the cloud's PLY header `[repo: utils/preflight.py]` | ✅ **RESOLVED.** All four clouds clear `n_bud = 200 000`. Post-CD-26 (the parallax filter regenerated every cloud): Panama 244 197, IUI3 288 609, Curasao 292 707, JapaneseGardens 293 642 `[measured n=1/scene, 06-implementation-deltas §6.8]`. The pre-filter clouds were 299 368 – 471 531; filtering shrank them and made them more uniform (1.58× spread → 1.20×), but **narrowed the margin on the binding scene** — Panama is now 81.9% of its cloud rather than 66.8%, so M2 removes 18.1% of the M1 population there rather than 33%. That is the lever A4 and A7 have to work with |
...
- ~~**No mechanism guarantees the budget binds.**~~ **RESOLVED.** Preflight reads the point
  ...
  run consumed it.
- **The medium re-warm-up does not re-identify `β`, and this is now measured.** The `[PI]`
  burst was specified by analogy with SeaSplat's existing warm-up, and 200 steps proved
  insufficient in all six collapsed runs `[measured n=12]`. The burst has been
  unconditionally enabled since it was written, so every A2 figure in this campaign is a
  *post-burst* figure — there is no unmitigated arm to compare against, and the negative
  result is about the remedy's adequacy, not its presence. Why it fails is not established;
  the candidates are burst length, the frozen-geometry assumption, and the possibility that
  the per-frame normalisation makes β unrecoverable by any amount of re-fitting once the
  depth range has moved. **The last would mean the remedy is misconceived rather than
  under-budgeted**, and it is the one worth testing next.
