...
| 14f | **`[PI]`** Sweep the **cross-frame depth-range distribution** at every simplification boundary and at `zsweep_interval` (CD-27) — `zr_cv` is the quantity `β` actually tracks | `z_min`/`z_max` come from the single view sampled at that iteration, so `z_range` is one draw from the distribution the central hypothesis is about rather than a statistic of it. The quantity was never measured in the campaign. **Runs predating the instrument cannot be retrofitted**; report the prediction as untested, never as unsupported |
| 14g | **`[PI]`** Declare the **equivalence margin** for A0 ≡ SS *before* S0 runs: PSNR within **±1.0 dB** per scene (pooled convention), LPIPS within **±0.02**, primitive count within **±30%** | An equivalence claim needs a margin fixed in advance; a p-value afterwards cannot license the word. The margins are set to what this design can certify rather than to what would be impressive: at n=3 per side the PSNR difference resolves to ±0.36–1.14 dB, so ±1.0 dB is certifiable on three of four scenes. **The count margin is deliberately loose and that is the finding** — with a per-scene CV of 6.0–29.3%, n=3 cannot certify better than ±9.6% to ±46.9%, so a tight count equivalence is not available at this sample size and should not be implied `[09-supervisory-review-ii §3]` |
